...
void mks_probe_check(void) {
    ...
    //     if(probe_ctrl.status != PROBE_GET_STATUS) {
    //         probe_ctrl.status = probe_ctrl.status;
    //     }
    //     return ;
    // };
    ...
    switch(probe_ctrl.status) {
        ...
        case PROBE_IS_WRONG:
            if(PROBE_READ() == PROBE_READ_LOW) { // 再次确定触发了
                ...
                if( probe_ctrl.probe_time_end -  probe_ctrl.probe_time_start >= PROBE_DELAY_TIME) {  // 防抖延时
